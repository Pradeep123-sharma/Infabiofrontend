...
css = css.replace(/            display: flex;\r?\n            justify-content: center;/g, '');
css = css.replace(/            width: 1440px;/g, '            width: 100%;\r\n            max-width: 1440px;\r\n            margin: 0 auto;\r\n            overflow-x: hidden;');
...
const startMarker = '<!-- Image 1 & 4 & 6: From Defense to Domination (Case Studies) -->';
const endMarker = '        <!-- Image 2: Why Clients Trust Us -->';

const sIdx = html.indexOf(startMarker);
const eIdx = html.indexOf(endMarker);

if (sIdx !== -1 && eIdx !== -1) {
    const replacement = `<!-- Image 1 & 4 & 6: From Defense to Domination (Case Studies) -->
...
`;
    ...
    const newHtml = html.substring(0, sIdx) + replacement + "\n" + html.substring(eIdx);
    fs.writeFileSync(htmlPath, newHtml, 'utf8');
    console.log("Fixed html");
} else {
    console.log("Could not find markers", !!sIdx, !!eIdx);
}
